...
export const AjinkyaAutoComplete = ({city , setcity} : {city : string, setcity : any}) => {
    ...
    return (
        <div className=''>
            ...
            {
                <>
                    <div onClick={() => { setc([...Cities.map((city) => city.name)]); setflim(true) }} className='w-[60vw] bg-white p-1 rounded'>{city !== "" ? city : "Select City"}</div>
                </>
            }
        </div>
    )
