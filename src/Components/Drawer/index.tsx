...
export const Drawer = () => {
  ...
  const { username, name, email_id, mobile_no, location } = useSelector(
  ...
  return (
    <div
      ...
      className={`absolute top-0 
        bg-white text-slate-500 h-screen ${
          showDrawer === 0 ? "hidden" : "drawer-width"
        } rounded-r-xl`}
    >
      <div id="inside-content" className="hidden">
        <div className="flex flex-col gap-5 pt-3">
          <div className="flex flex-wrap justify-between pr-5 items-center font-semibold">
            ...
            <CloseOutlined
              ...
              onClick={() => upDateDrawerBackTo0(dispatch)}
            />
          </div>
          <div className="flex bg-slate-100 p-5 flex-col justify-center items-center gap-3">
            <img
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTTtMfy4lLEvhrLkv8FRibbj2rJaHYFEqTPmQ&usqp=CAU"
              className="w-20 h-20 rounded-full shadow-lg border-2 "
            ></img>
            <section className="px-3 h-7 text-center  shadow-lg border">
              {name ?? username}
            </section>
            <section className="px-3 h-7 text-center  shadow-lg border">
              {email_id}
