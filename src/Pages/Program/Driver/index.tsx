...
export const Driver = () => {
  ...
  const { cab_id, hourly_rate, kms_rate: kmr } = useSelector((state: any) => state.session)
  ...
  const Handle$OnClick$DriverComponent = (driver: any) => {
    ...
    dispatch(sessionActions.addSessionHourlyRate({ hourly_rate: hourly_rate + driver.rate_per_hrs }))
    dispatch(sessionActions.addSessionKmsRate({ kms_rate: kmr + driver.rate_per_km }))
    if (cab_id === "") {
      navigate("/cabs")
    }
    else {
    ...
    <div style={{ backgroundImage: "url(https://images.unsplash.com/photo-1519681393784-d120267933ba?ixi)", backgroundPosition: "center" }} className='text-white h-screen'>
      ...
      <div className='' >
        <div className='flex w-full justify-evenly p-6 text-black'>
          <select className='px-4 py-2 rounded-lg' onChange={(ev) => setrating(Number(ev.target.value))} name=''>
          ...
          <select className='px-4 py-2 rounded-lg' onChange={(ev) => setexperience_years(Number(ev.target.value))} name=''>
      ...
      <div className='flex gap-10 flex-wrap justify-center'>
        ...
        {
          totalChunk.current !== 0 && drivers.map((driver: any) => {
            return (
              <div style={{ backgroundColor: "rgba(17, 25, 40, 0.20)", backdropFilter: "blur(16px) saturate(200%)" }} className="rounded-xl p-3 w-84" onClick={() => Handle$OnClick$DriverComponent(driver)}>
